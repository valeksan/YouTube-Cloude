"""Local shadow of the p4a kivy recipe.

p4a master's kivy recipe lists `requests` in python_depends, which causes
p4a to pip-install requests (and its charset-normalizer dependency) into the
broken android venv (fails on Python 3.14 with "wheel is not a supported
wheel on this platform"). To keep this build working we remove `requests`
from python_depends and instead install requests from a local recipe built
from source (see recipes/requests). The remaining python_depends
(certifi, chardet, idna, urllib3) satisfy requests 2.25.1's runtime deps.
"""
